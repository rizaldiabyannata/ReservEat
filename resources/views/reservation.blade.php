...
<head>
    ...
    <title>Reservation Selection</title>
</head>
...
<body class="font-['Montserrat'] bg-gray-100">
    ...
    <div class="container mx-auto py-8 px-4 md:px-8 lg:px-16">
        ...
        <div class="bg-white p-6 rounded-lg shadow-md mb-8">
            ...
            <p class="text-gray-700 mb-4">Location: {{$restaurant->address}}</p>
        </div>

        <div class="bg-white p-6 rounded-lg shadow-md">
            <h4 class="text-xl font-semibold mb-2">Reservation Details</h4>
            <form action="{{'/restaurants/' . $restaurant->id . '/reservation'}}" id="reservation-form" method="post">
                @csrf
                <div class="mb-4">
                    <label for="date" class="block text-gray-700 font-semibold mb-2">Date:</label>
                    <input type="datetime-local" id="date" name="date" class="w-full border border-gray-300 rounded-md p-2" required>
                </div>
                <div class="mb-4">
                    <label for="guests" class="block text-gray-700 font-semibold mb-2">Number of Guests:</label>
                    <input type="number" id="guests" name="guests" min="1" class="w-full border border-gray-300 rounded-md p-2" required>
                </div>
                <div class="mb-4">
                    <label for="table" class="block text-gray-700 font-semibold mb-2">Table Number:</label>
                    <input type="number" id="table" name="table" min="1" max="{{$restaurant->number_of_tables}}" class="w-full border border-gray-300 rounded-md p-2" required>
                </div>
